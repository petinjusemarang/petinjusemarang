-- SAMLONG CDID — Merged All-in-One
-- Semua script di-inline, tidak ada loadstring untuk file lokal
-- Ganti ACTIVE_MODE untuk memilih mode (auto-select: coming soon)
...
-- ═══════════════════════════════════
--  MODE SELECTION
--  "minigame" | "race" | "fishing" | "joki_uang"
-- ═══════════════════════════════════
local ACTIVE_MODE = "minigame"  -- TODO: auto-detect
...
local function sheetsRequest(url)
    pcall(function()
        ...
        if req then req({ Url = url, Method = "GET" })
...
-- ═══════════════════════════════════
--  SERVER LOCK (on start)
-- ═══════════════════════════════════
serverLock()

-- ═══════════════════════════════════
--  JUMP / NOJUMP (inlined dari jump.lua / nojump.lua)
-- ═══════════════════════════════════
...
-- ═══════════════════════════════════
--  MODE: MINIGAME (inlined dari samlongmini.lua)
-- ═══════════════════════════════════
local function startMinigame()
    ...
    local jumpBtn = Instance.new("TextButton", banner)
    jumpBtn.Size                   = UDim2.new(0.3, 0, 1, -10)
    jumpBtn.Position               = UDim2.new(0.05, 0, 0, 5)
    jumpBtn.Font                   = Enum.Font.GothamBold
    jumpBtn.TextSize               = 20
    jumpBtn.BackgroundColor3       = Color3.fromRGB(40, 180, 90)
    jumpBtn.TextColor3             = Color3.new(1, 1, 1)
    jumpBtn.Text                   = "JUMP"
    jumpBtn.BorderSizePixel        = 0
    jumpBtn.TextStrokeTransparency = 0.3
    jumpBtn.TextStrokeColor3       = Color3.new(0, 0, 0)

    local nojumpBtn = Instance.new("TextButton", banner)
    nojumpBtn.Size                   = UDim2.new(0.3, 0, 1, -10)
    nojumpBtn.Position               = UDim2.new(0.65, 0, 0, 5)
    nojumpBtn.Font                   = Enum.Font.GothamBold
    nojumpBtn.TextSize               = 20
    nojumpBtn.BackgroundColor3       = Color3.fromRGB(255, 90, 70)
    nojumpBtn.TextColor3             = Color3.new(1, 1, 1)
    nojumpBtn.Text                   = "NOJUMP"
    nojumpBtn.BorderSizePixel        = 0
    nojumpBtn.TextStrokeTransparency = 0.3
    nojumpBtn.TextStrokeColor3       = Color3.new(0, 0, 0)
    ...
    -- INIT: Google Sheets + Railway API
    ...
    -- JUMP / NOJUMP → pakai fungsi yang sudah di-inline di atas
    jumpBtn.MouseButton1Click:Connect(function() startJumpLoop() end)
    nojumpBtn.MouseButton1Click:Connect(function() startNoJumpLoop() end)
    ...
    -- Periodic Sheets + API update (20 menit)
    task.spawn(function()
        while true do
            task.wait(1200)
            local guiInst = player:FindFirstChild("PlayerGui")
...
-- ═══════════════════════════════════
--  MODE: RACE (inlined dari danur1.lua — AUTO RACE v8.3)
-- ═══════════════════════════════════
local function startRace()
    ...
    local startBtnR = Instance.new("TextButton", botBar)
    startBtnR.Size             = UDim2.new(1, -20, 0, 26)
    startBtnR.Position         = UDim2.new(0, 10, 0, 5)
    startBtnR.BackgroundColor3 = Color3.fromRGB(35, 140, 70)
    startBtnR.Text             = "START"
    startBtnR.TextColor3       = Color3.fromRGB(255, 255, 255)
    startBtnR.Font             = Enum.Font.GothamBold
    startBtnR.AutoButtonColor  = true
    corner(startBtnR, 8)

    local winBtn = Instance.new("TextButton", botBar)
    winBtn.Size             = UDim2.new(0.5, -15, 0, 22)
    winBtn.Position         = UDim2.new(0, 10, 0, 36)
    winBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 65)
    winBtn.Text             = "WIN"
    winBtn.TextColor3       = Color3.fromRGB(255, 255, 255)
    winBtn.Font             = Enum.Font.GothamBold
    winBtn.AutoButtonColor  = true
    corner(winBtn, 6)

    local loseBtn = Instance.new("TextButton", botBar)
    loseBtn.Size             = UDim2.new(0.5, -15, 0, 22)
    loseBtn.Position         = UDim2.new(0.5, 5, 0, 36)
    loseBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 65)
    loseBtn.Text             = "LOSE"
    loseBtn.TextColor3       = Color3.fromRGB(255, 255, 255)
    loseBtn.Font             = Enum.Font.GothamBold
    loseBtn.AutoButtonColor  = true
    corner(loseBtn, 6)

    local infoLblR = Instance.new("TextLabel", botBar)
    infoLblR.Size                   = UDim2.new(1, -20, 0, 10)
    infoLblR.Position               = UDim2.new(0, 10, 0, 62)
    infoLblR.BackgroundTransparency = 1
    infoLblR.TextColor3             = Color3.fromRGB(55, 55, 80)
    infoLblR.Font                   = Enum.Font.Gotham
    infoLblR.TextXAlignment         = Enum.TextXAlignment.Center
    infoLblR.Text                   = ""
    ...
    local function refreshRaceGUI()
        ...
        if RUNNING then
            ...
            startBtnR.Text       = "STOP"
            startBtnR.BackgroundColor3 = Color3.fromRGB(160, 40, 40)
        else
        ...
        infoLblR.Text = string.format("v8.3 | %s | BV %d/%d", MODE_RACE == "WIN" and "HOST" or "JOIN", SPEED.WIN, SPEED.LOSE)
    end
    refreshRaceGUI()

    startBtnR.MouseButton1Click:Connect(function()
        RUNNING = not RUNNING
        if RUNNING then SESSION_START = os.clock(); POINTS_AT_START = getPointsNum() end
        STATUS_TEXT = RUNNING and "Starting..." or "Stopped"
        refreshRaceGUI()
    end)
    winBtn.MouseButton1Click:Connect(function()  MODE_RACE = "WIN";  refreshRaceGUI() end)
    loseBtn.MouseButton1Click:Connect(function() MODE_RACE = "LOSE"; refreshRaceGUI() end)
    task.spawn(function() while true do task.wait(1); refreshRaceGUI() end end)

    -- Discord webhook (tiap 30 menit)
    ...
    -- Google Sheets + Railway API (tiap 10 menit)
    task.spawn(function()
        task.wait(5)
...
-- ═══════════════════════════════════
--  MODE: FISHING (inlined dari samlongmancing.lua)
-- ═══════════════════════════════════
local function startFishing()
    local guiService = player:WaitForChild("PlayerGui")
    local sellRemote = rp:WaitForChild("NetworkContainer"):WaitForChild("RemoteEvents"):WaitForChild("EventSellShop")
    ...
    local function updatePoints()
        if headerText and headerText.Text then
            local point = headerText.Text:gsub("%D", "")
            if point ~= "" then
                if point ~= lastPoint then lastPoint = point; lastCatchTime = tick() end
    ...
    startBtn.MouseButton1Click:Connect(function()
        startBtn:Destroy()
        -- Script fishing automation (external, tidak tersedia lokal)
...
-- ═══════════════════════════════════
--  MODE: JOKI UANG (dari samlongfarm.lua)
--  External luarmor script tidak bisa di-inline
-- ═══════════════════════════════════
local function startJokiUang()
    ...
    local function makeBtn(size, pos, bgColor, text)
        local btn = Instance.new("TextButton", jokiGui)
        btn.Size             = size
        btn.Position         = pos
    ...
    local startBtn = makeBtn(
        ...
        "Mulai (Langsung start aaja)"
    )

    local showUIBtn = makeBtn(
    ...
    showUIBtn.MouseButton1Click:Connect(function()
        ...
        -- MONEY OVERLAY (inlined dari samlongfarm.lua → mountMoneyOverlay)
        ...
        task.spawn(function()
            while true do
                ...
                if elapsed >= 360 then ng.Visible = true; break end
            end
        end)

        -- Sheets + API (tiap 15 menit)
        task.spawn(function()
            task.wait(3)
...
--  RUN ACTIVE MODE
-- ═══════════════════════════════════
if     ACTIVE_MODE == "minigame"  then startMinigame()
elseif ACTIVE_MODE == "race"      then startRace()
elseif ACTIVE_MODE == "fishing"   then startFishing()
elseif ACTIVE_MODE == "joki_uang" then startJokiUang()
end
